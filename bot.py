...
# taken from https://github.com/Rapptz/discord.py/blob/v2.3.2/examples/advanced_startup.py
class JeopardyBot(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        ...
        if self.testing_guild_id:
            ...
            # print(f"{self.get_command('help')}")
            # self.tree.add_command(self.get_command('help'))
...
async def main():
    ...
    # Here we have a web client which does cleanup at exit.
    # We also have our bot
    ...
    async with ClientSession() as our_client: # , asyncpg.create_pool(user='postgres', command_timeout=30) as pool:
        ...
        async with JeopardyBot(command_prefix=commands.when_mentioned,
